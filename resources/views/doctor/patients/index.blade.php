    ...
    <div class="space-y-6">
        ...
        @if ($canManagePatients)
            <div class="flex justify-end">
                <a href="{{ route('nurse-regular.patients.create') }}" class="rounded-lg bg-cyan-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-cyan-800">
                    Tambah Patient
                </a>
            </div>
        @endif
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form method="GET" action="{{ route('nurse-regular.patients.index') }}" class="grid gap-3 lg:grid-cols-5 lg:items-end">
                ...
                <div class="flex gap-2 lg:col-start-5">
                    ...
                    <a href="{{ route('nurse-regular.patients.index') }}" class="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </section>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($patients as $patient)
                            <tr>
                                ...
                                <td class="px-5 py-4">
                                    <a href="{{ route('nurse-regular.patients.show', $patient) }}" class="font-semibold text-cyan-800 hover:underline">
                                        {{ $patient->name }}
                                    </a>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('nurse-regular.patients.show', $patient) }}" class="rounded-md border border-slate-200 px-3 py-1.5 font-medium text-slate-700 hover:bg-slate-50">Detail</a>
                                        @if ($canManagePatients)
                                            <a href="{{ route('nurse-regular.patients.edit', $patient) }}" class="rounded-md border border-cyan-200 px-3 py-1.5 font-medium text-cyan-700 hover:bg-cyan-50">Edit</a>
                                            <form method="POST" action="{{ route('nurse-regular.patients.destroy', $patient) }}" onsubmit="return confirm('Hapus data patient ini?');">
                                                @csrf
                                                @method('DELETE')
